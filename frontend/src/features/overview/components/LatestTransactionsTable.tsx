...
export const LatestTransactionsTable = ({
  ...
  return (
    <div className="col-span-4 rounded-[10px] bg-(--card-background) px-[18.8px] py-[16.8px] text-left outline-1 outline-(--content-outline)">
      <h3 className="mb-3 text-sm font-medium text-(--text-primary-white)">
        Naujausios transakcijos
      </h3>

      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-xs text-(--text-gray-400)">
        ...
        <tbody>
          {latestTransactions.map((t) => (
            <tr
              key={t.id}
              className="border-t border-(--content-outline)"
            >
              <td className="py-3 text-(--text-gray-400)">
                {t.transactionDate.toString()}
              </td>

              <td className="py-3 text-(--text-primary-white)">
                {t.merchantName}
              </td>
